...
class SecurityConfig(BaseModel):
    ...
    jwt_access_token_expire_minutes: int = Field(default=15, description="JWT access token expiry in minutes")
    jwt_refresh_token_expire_days: int = Field(default=30, description="JWT refresh token expiry in days")
    ...
    auth_rate_limit_requests: int = Field(default=10, description="Auth requests per window")
    auth_rate_limit_window: int = Field(default=900, description="Auth rate limit window in seconds (15 minutes)")
    
    # OAuth2 Configuration (placeholder)
    oauth2_enabled: bool = Field(default=False, description="Enable OAuth2 authentication")
    oauth2_providers: List[str] = Field(default=[], description="Supported OAuth2 providers")
...
class SecurityHeaders(BaseModel):
    ...
    content_security_policy: str = "default-src 'self'; connect-src 'self'; style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://fastapi.tiangolo.com; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; img-src 'self' data: https://fastapi.tiangolo.com https://cdn.jsdelivr.net; font-src 'self' https://cdn.jsdelivr.net"

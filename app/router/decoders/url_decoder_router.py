...
async def decode_url_text(
    ...
    errors: str = Query(default="replace", enum=["strict", "ignore", "replace"], description="Error handling"),
    ...
    try:
        if plus_spaces:
            decoded_text = service.decode_plus(encoded_text, encoding=encoding, errors=errors)
        else:
            decoded_text = await service.decode(encoded_text, encoding=encoding, errors=errors)
...
async def decode_url_file(
    ...
    """
    try:
        decoded_content = await service.decode_file(file, encoding=encoding, errors=errors)
...
async def decode_url_params(
    ...
    keep_blank_values: bool = Query(default=False, description="Keep blank parameter values"),
    strict_parsing: bool = Query(default=False, description="Use strict parsing"),
    encoding: str = Query(default="utf-8", description="Character encoding"),
    max_num_fields: Optional[int] = Query(default=None, description="Maximum number of fields"),
    service: URLDecoderService = Depends(get_url_decoder_service)
) -> JSONResponse:
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Parameter parsing failed: {str(e)}")

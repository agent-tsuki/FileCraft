...
from app.services.decoders.base64_decoder import (
    Base64DecoderService,
    get_base64_decoder_service,
)
...
async def decode_base64_file(
    ...
    url_safe: Optional[bool] = Query(
        default=None, description="Force URL-safe decoding (auto-detect if None)"
    ),
    ...
    try:
        if return_stream:
            ...
            decoded_stream = service.decode_stream(
                file, url_safe=url_safe, validate=validate
            )
...
async def decode_base64_text(
    ...
    output_format: str = Query(
        default="text", enum=["text", "hex", "binary"], description="Output format"
    ),
    ...
    try:
        decoded_bytes = await service.decode(
            encoded_text, url_safe=url_safe, validate=validate
        )

        result = {
            "input": (
                encoded_text[:100] + "..." if len(encoded_text) > 100 else encoded_text
            ),
            "input_length": len(encoded_text),
            "decoded_size": len(decoded_bytes),
            "url_safe_detected": service._is_url_safe_base64(

...
async def decode_hex_text(
    ...
    ignore_whitespace: bool = Query(
        default=True, description="Ignore whitespace in hex"
    ),
    ignore_separators: bool = Query(
        default=True, description="Ignore common separators"
    ),
    strict: bool = Query(default=False, description="Strict hex validation"),
    output_format: str = Query(
        default="text", enum=["text", "binary", "base64"], description="Output format"
    ),
...
async def decode_hex_to_text(
    ...
    try:
        decoded_text = await service.decode_to_text(
            hex_text, encoding=encoding, ignore_whitespace=True, ignore_separators=True
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Hex to text decoding failed: {str(e)}"
        )
...
async def decode_hex_with_length(
    ...
    try:
        ...
        cleaned_hex = service._clean_hex_string(
            hex_data, ignore_whitespace=True, ignore_separators=True
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Length-prefixed hex decoding failed: {str(e)}"
        )
...
async def analyze_hex_content(
    ...
    try:
        cleaned_hex = service._clean_hex_string(
            hex_text, ignore_whitespace=True, ignore_separators=True
        )
        ...
        if analysis["is_valid_hex"] and len(cleaned_hex) >= 2:
            try:
                ...
                analysis["sample_decoded"] = {
                    ...
                    "ascii_preview": "".join(
                        chr(b) if 32 <= b <= 126 else "." for b in sample_bytes
                    ),
                }
            except:

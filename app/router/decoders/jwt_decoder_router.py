...
@jwt_decoder_router.post(
    "/token", summary="Decode JWT token", description="Decode and verify JWT token"
)
async def decode_jwt_token(
    token: str = Form(..., description="JWT token to decode"),
    secret: str = Form(
        default="your-secret-key", description="Secret key for verification"
    ),
    ...
    raw_output: bool = Query(
        default=False, description="Return raw binary data if payload contains base64"
    ),
...
async def decode_jwt_file(
    ...
    try:
        payload = await service.decode_file(
            file, secret=secret, algorithm=algorithm, verify=verify
        )
...
async def decode_jwt_header(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"JWT header decoding failed: {str(e)}"
        )
...
async def validate_jwt_format(
    ...
    try:
        ...
        return JSONResponse(content={"valid_format": False, "error": str(e)})

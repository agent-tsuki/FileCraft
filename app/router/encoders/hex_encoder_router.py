...
async def encode_text_to_hex(
    ...
    try:
        ...
        encoded_hex = await service.encode(
            byte_data, uppercase=uppercase, separator=separator, prefix=prefix
        )
    ...
    try:
        if return_stream:
            ...
            hex_stream = service.encode_stream(
                file, uppercase=uppercase, separator=separator, prefix=prefix
            )
...
async def encode_ascii_to_hex(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Text contains non-ASCII characters"
            )

        encoded_hex = await service.encode_ascii_hex(
            ascii_text, uppercase=uppercase, separator=separator, prefix=prefix
        )
        ...
        for char in ascii_text:
            ...
            char_mapping.append({"char": char, "ascii_code": ord(char), "hex": hex_val})
        ...
        raise HTTPException(
            status_code=500, detail=f"ASCII hex encoding failed: {str(e)}"
        )
...
async def encode_with_length_prefix(
    ...
    try:
        encoded_hex = await service.encode_with_length(
            data, encoding=encoding, uppercase=uppercase
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Length-prefixed encoding failed: {str(e)}"
        )

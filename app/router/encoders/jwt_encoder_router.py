...
async def encode_payload_to_jwt(
    ...
    exp_minutes: Optional[int] = Query(
        default=None, description="Expiration time in minutes"
    ),
...
@jwt_encoder_router.post(
    "/text", summary="Encode text to JWT", description="Create JWT token with text data"
)
async def encode_text_to_jwt(
    ...
    try:
        token = await service.encode(
            text, secret=secret, algorithm=algorithm, exp_minutes=exp_minutes
        )
...
async def encode_file_to_jwt(
    ...
    try:
        token = await service.encode_file(
            file, secret=secret, algorithm=algorithm, exp_minutes=exp_minutes
        )

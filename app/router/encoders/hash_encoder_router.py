...
from app.services.encoders.hash_encoder import HashEncoderService, get_hash_encoder_service
...
async def hash_text(
    ...
    output_format: str = Query(default="hex", enum=["hex", "base64", "bytes"], description="Output format"),
...
async def hash_file(
    ...
    output_format: str = Query(default="hex", enum=["hex", "base64", "bytes"], description="Output format"),
    salt: Optional[str] = Query(default=None, description="Salt for hashing"),
    use_streaming: bool = Query(default=True, description="Use streaming for large files"),
    service: HashEncoderService = Depends(get_hash_encoder_service)
) -> JSONResponse:
    """
...
@hash_encoder_router.post(
    "/verify",
    summary="Verify hash",
    description="Verify data against expected hash"
)
async def verify_hash(
    data: str = Form(..., description="Data to verify"),
    expected_hash: str = Form(..., description="Expected hash value"),
    algorithm: str = Query(default="sha256", description="Hash algorithm"),
    output_format: str = Query(default="hex", enum=["hex", "base64", "bytes"], description="Hash format"),
...
async def list_hash_algorithms(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Algorithm listing failed: {str(e)}")
...
async def get_algorithm_info(
    ...
    try:
        ...
        info["security_notes"] = security_notes.get(algorithm_name.lower(), "No specific notes")
        info["common_use_cases"] = use_cases.get(algorithm_name.lower(), ["General hashing"])

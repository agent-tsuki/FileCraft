...
async def encode_text_to_url(
    ...
    plus_encoding: bool = Query(
        default=False, description="Use + for spaces (quote_plus)"
    ),
...
async def encode_params_to_url(
    ...
    try:
        ...
        if not isinstance(param_dict, dict):
            raise HTTPException(
                status_code=400, detail="Parameters must be a JSON object"
            )

        encoded_params = service.encode_query_params(
            param_dict, doseq=doseq, safe=safe, encoding=encoding
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Parameter encoding failed: {str(e)}"
        )
...
async def encode_url_component(
    ...
    component_type: str = Query(
        default="query",
        enum=["path", "query", "fragment"],
        description="Type of URL component",
    ),
    service: URLEncoderService = Depends(get_url_encoder_service),
) -> JSONResponse:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Component encoding failed: {str(e)}"
        )

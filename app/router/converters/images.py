...
async def convert_image_format(
    ...
    quality_preset: Optional[QualityPreset] = Query(
        default=None, description="Quality preset (overrides quality if specified)"
    ),
    ...
    resize_width: Optional[int] = Query(
        default=None, ge=1, le=8000, description="Target width in pixels"
    ),
    resize_height: Optional[int] = Query(
        default=None, ge=1, le=8000, description="Target height in pixels"
    ),
    size_preset: Optional[SizePreset] = Query(
        default=None, description="Size preset (overrides width/height if specified)"
    ),
    maintain_aspect_ratio: bool = Query(
        default=True, description="Maintain aspect ratio when resizing"
    ),
    allow_upscale: bool = Query(
        default=False, description="Allow upscaling (enlarging) images"
    ),
...
async def batch_convert_images(
    ...
    optimization_level: OptimizationLevel = Query(
        default=OptimizationLevel.MEDIUM, description="Optimization level"
    ),
    image_service: ImageService = Depends(get_image_service),
) -> JSONResponse:
    """

...
async def convert_audio_format(
    ...
    bitrate: Optional[int] = Query(
        default=128, ge=32, le=1411, description="Audio bitrate in kbps (32-1411)"
    ),
    ...
    use_async: bool = Query(
        default=False, description="Use background processing (returns task ID)"
    ),
    ...
    **Returns:**
    ...
    content_type = content_type_map.get(
        target_format.value, f"audio/{target_format.value}"
    )
...
async def batch_convert_audio(
    audio_files: List[UploadFile] = File(
        ..., description="List of audio files to convert"
    ),
    target_format: AudioFormat = Form(
        ..., description="Target format for all audio files"
    ),
    bitrate: int = Query(
        default=128, ge=32, le=1411, description="Audio bitrate in kbps"
    ),
    quality_preset: Optional[AudioQualityPreset] = Query(
        default=None, description="Quality preset"
    ),
    audio_service: AudioService = Depends(get_audio_service),
) -> JSONResponse:
    """
...
async def apply_audio_effects(
    ...
    # Effect parameters
    normalize_headroom: float = Query(
        default=0.1, ge=0.0, le=1.0, description="Normalization headroom"
    ),
    compress_threshold: float = Query(
        default=-20.0, ge=-60.0, le=0.0, description="Compression threshold (dB)"
    ),
    compress_ratio: float = Query(
        default=4.0, ge=1.0, le=20.0, description="Compression ratio"
    ),
    fade_duration: int = Query(
        default=1000, ge=100, le=10000, description="Fade duration (ms)"
    ),
    volume_change: float = Query(
        default=0.0, ge=-60.0, le=20.0, description="Volume change (dB)"
    ),
    speed_factor: float = Query(
        default=1.0, ge=0.1, le=3.0, description="Speed change factor"
    ),
    audio_service: AudioService = Depends(get_audio_service),
) -> Union[StreamingResponse, JSONResponse]:
    """
...
async def extract_audio_segments(
    ...
    end_time: Optional[float] = Query(
        default=None, ge=0.0, description="End time in seconds (optional)"
    ),
    duration: Optional[float] = Query(
        default=None, ge=0.1, le=3600.0, description="Duration in seconds (optional)"
    ),
    audio_service: AudioService = Depends(get_audio_service),
) -> Union[StreamingResponse, JSONResponse]:
    """

...
async def convert_video_format(
    ...
    codec: Optional[VideoCodec] = Query(
        default=VideoCodec.auto, description="Video codec to use for encoding"
    ),
    bitrate: Optional[str] = Query(
        default=None, description="Video bitrate (e.g., '1000k', '5M')"
    ),
    # Resolution options
    width: Optional[int] = Query(
        default=None, ge=64, le=7680, description="Target width in pixels"
    ),
    height: Optional[int] = Query(
        default=None, ge=64, le=4320, description="Target height in pixels"
    ),
    frame_rate: Optional[float] = Query(
        default=None, ge=12.0, le=120.0, description="Target frame rate (fps)"
    ),
...
async def batch_convert_videos(
    ...
    quality_preset: Optional[VideoQualityPreset] = Query(
        default=VideoQualityPreset.hd, description="Quality preset for all videos"
    ),
    codec: Optional[VideoCodec] = Query(
        default=VideoCodec.auto, description="Video codec to use"
    ),
    video_service: VideoService = Depends(get_video_service),
) -> JSONResponse:
    """
...
async def extract_audio_from_video(
    ...
    audio_format: AudioFormat = Query(
        default=AudioFormat.mp3, description="Output audio format"
    ),
    ...
    **Parameters:**
    ...
    result = await video_service.extract_audio_from_video(
        video, audio_format.value, audio_bitrate
    )
...
async def generate_video_thumbnail(
    ...
    width: int = Query(default=320, ge=64, le=1920, description="Thumbnail width"),
    height: int = Query(default=240, ge=64, le=1080, description="Thumbnail height"),
    image_format: str = Query(
        default="jpg", regex="^(jpg|jpeg|png|webp)$", description="Output image format"
    ),
    video_service: VideoService = Depends(get_video_service),
) -> Union[StreamingResponse, JSONResponse]:
    """
    ...
    **Parameters:**
    ...
    result = await video_service.generate_thumbnail(
        video, timestamp, width, height, image_format
    )
...
async def get_video_task_status(task_id: str) -> JSONResponse:
    ...
    **Task Types:**
    ...
    try:
        ...
        return JSONResponse(
            content={"error": "Task system not available", "task_id": task_id},
            status_code=503,
        )

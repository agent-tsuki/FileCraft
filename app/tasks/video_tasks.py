...
def convert_video_task(
    ...
    try:
        ...
        self.update_state(state='PROGRESS', meta={'progress': 0, 'status': 'Starting conversion'})
        
        # Create temporary files
        with tempfile.NamedTemporaryFile(delete=False, suffix=f".{filename.split('.')[-1]}") as input_temp:
            input_temp.write(video_data)
            input_temp.flush()
            input_path = input_temp.name
        
        with tempfile.NamedTemporaryFile(delete=False, suffix=f".{target_format}") as output_temp:
            output_path = output_temp.name
        
        try:
            # Update progress
            self.update_state(state='PROGRESS', meta={'progress': 20, 'status': 'Processing video'})
            ...
            # Update progress
            self.update_state(state='PROGRESS', meta={'progress': 50, 'status': 'Converting video'})
    ...
    try:
        ...
        for i, video_info in enumerate(videos_data):
            try:
                ...
                logger.error(f"Error converting video {video_info['filename']}: {str(e)}")
                failed_conversions += 1
                results.append({
                    'status': 'FAILURE',
                    'error': str(e),
...
def extract_audio_task(
    ...
    try:
        ...
        # Update task progress
        self.update_state(state='PROGRESS', meta={'progress': 0, 'status': 'Starting audio extraction'})
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=f".{audio_format}") as output_temp:
            output_path = output_temp.name
        
        try:
            # Update progress
            self.update_state(state='PROGRESS', meta={'progress': 50, 'status': 'Extracting audio'})
...
def generate_thumbnail_task(
    ...
    try:
        ...
        # Update task progress
        self.update_state(state='PROGRESS', meta={'progress': 0, 'status': 'Starting thumbnail generation'})
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=f".{image_format}") as output_temp:
            output_path = output_temp.name
        
        try:
            # Update progress
            self.update_state(state='PROGRESS', meta={'progress': 50, 'status': 'Generating thumbnail'})

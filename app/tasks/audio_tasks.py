...
def convert_audio_async(
    ...
    try:
        ...
        try:
            ...
            self.update_state(state="PROCESSING", meta={"step": "loaded", "original_info": original_info})
            
            # Apply conversions
            if channels:
                audio = audio.set_channels(channels)
                self.update_state(state="PROCESSING", meta={"step": "channels_converted"})
            
            if sample_rate:
                audio = audio.set_frame_rate(sample_rate)
                self.update_state(state="PROCESSING", meta={"step": "sample_rate_converted"})
...
@celery_app.task(
    bind=True,
    name="app.tasks.audio_tasks.batch_convert_audio",
    max_retries=2
)
def batch_convert_audio(
    self,
    audio_files_data: List[Dict[str, Any]],
    ...
    try:
        ...
        try:
            ...
            basic_info = {
                ...
                "bitrate_estimate": _estimate_bitrate(len(audio_data), len(audio) / 1000.0)
            }
            ...
            try:
                ...
                if LOUDNORM_AVAILABLE:
                    try:
                        ...
                        advanced_info["loudness_lufs"] = float(loudness) if not np.isnan(loudness) else None
                    except Exception:
    ...
    try:
        ...
        try:
            ...
            if optimization_type == "size":
                ...
                if target_size_kb:
                    optimized_audio = _optimize_for_target_size(audio, target_size_kb * 1024)
                else:
...
def _apply_effect(audio: "AudioSegment", effect: str, params: Dict[str, Any]) -> "AudioSegment":
...
def _optimize_for_target_size(audio: "AudioSegment", target_bytes: int) -> "AudioSegment":

...
from app.helpers.constants import (
    VIDEO_FORMATS, SUPPORTED_VIDEO_OUTPUT_FORMATS, VIDEO_QUALITY_PRESETS,
...
class VideoService(BaseService):
    ...
    def __init__(self, config: AppConfig, validation_service: FileValidationService):
        ...
        self.executor = ThreadPoolExecutor(max_workers=2)  # Limited for video processing
    ...
    async def _validate_video_file(self, video_file: UploadFile) -> bytes:
        ...
        try:
            ...
            if file_type != "video":
                raise VideoProcessingError(f"Invalid file type: {file_type}. Expected video file.")
    ...
    async def convert_video_format(
        ...
        **kwargs
        ...
        return await self._convert_video_sync(
            video_file, target_format, quality_preset, codec, 
            bitrate, resolution, frame_rate, **kwargs
        )
    ...
    async def _convert_video_sync(
        ...
        **kwargs
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=f".{video_file.filename.split('.')[-1]}") as input_temp:
